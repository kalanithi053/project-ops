...
export function TaskActivity({
  ...
  const total = activity.data?.pages.at(-1)?.total;

  // Fires fetchNextPage() once the sentinel below the last loaded row scrolls
  // into view, so the next 10 entries load as the user scrolls further down.
  ...
  React.useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([observed]) => {
        if (observed?.isIntersecting) fetchNextPage();
      },
      { rootMargin: "200px" },
    );
    ...
    <section className="border-t border-border pt-4" aria-labelledby="activity-heading">
      ...
      {activity.isLoading ? (
        ...
        <>
            ...
            {entries.map((entry, index) => {
              ...
              return (
                <li key={entry.id} className="relative flex gap-3 pb-4 last:pb-0">
                  {(index < entries.length - 1 || hasNextPage) && (
                    <span
          ...
          {isFetchingNextPage && <ActivitySkeleton rows={3} />}
