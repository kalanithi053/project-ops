export function assignmentNotificationEmailTemplate(params: {
  ...
  const { entityLabel, entityName, projectName, assignmentRole, actionUrl } =
    params;
  const assignment = assignmentRole
    ? `as the ${assignmentRole} assignee`
    : `a ${entityLabel}`;
            ...
            <tr>
              <td style="padding:0 40px 24px 40px;">
                <p style="margin:0 0 8px 0;font-size:14px;color:#374151;">
                  You have been assigned ${assignment} in <strong>${projectName}</strong>:
                </p>
